...
export function SalesReferralCard({ code }: { code: string }) {
  const link = salesReferralUrl(env.NEXT_PUBLIC_SITE_URL, code)
  ...
  return (
    <Card padded="lg" variant="glass" className="min-w-0 space-y-4">
      <div>
        ...
        <p className="mt-2 break-all text-body-sm text-fg">{link}</p>
      </div>
