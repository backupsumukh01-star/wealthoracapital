 ...
 * Feed + timing controlled by Admin OS Live Activity Manager.
 */
export function LiveActivityToasts() {
  ...
  const feed = useMemo(() => {
    if (!ready || activity.seedItems.length === 0) return [...LIVE_ACTIVITY]
    return activity.seedItems
  }, [ready, activity.seedItems])
  ...
  useEffect(() => {
    ...
    if (ready && !activity.enabled) return

    let hideTimer: number | undefined
    const delay = activity.delayMs || 9000
    const showMs = Math.max(2000, 4800 / (activity.animationSpeed || 1))

    const showNext = () => {
      setIndex((i) => (i + 1) % Math.max(feed.length, 1))
      const item = feed[(index + 1) % Math.max(feed.length, 1)]
      const fromItem = (item as { type?: string } | undefined)?.type
      if (
        fromItem === 'deposit' ||
        fromItem === 'withdrawal' ||
        fromItem === 'profit' ||
  ...
  }, [prefersReducedMotion, ready, activity.enabled, activity.delayMs, activity.animationSpeed, feed.length])

  if (prefersReducedMotion) return null
  if (ready && !activity.enabled) return null

  const item = feed[index % feed.length] ?? feed[0]!
  const Icon =
    kind === 'deposit'
      ? ArrowDownToLine
      : kind === 'withdrawal'
      ...
      <AnimatePresence mode="wait">
        {visible ? (
          <motion.div
            ...
            transition={{ duration: 0.32 / (activity.animationSpeed || 1), ease: [0.16, 1, 0.3, 1] }}
